...
def extract_paragraphs(segment_outputs: list[dict]) -> list[dict]:
    """Collect paragraph metadata from all segment outputs."""
    all_paragraphs = []
    for seg in segment_outputs:
        all_paragraphs.extend(seg.get("paragraphs", []))
    return all_paragraphs


def split_words_by_paragraph(
    words: list[dict], paragraphs: list[dict]
) -> list[dict]:
    """Split flat word list into chunks using paragraph total_words counts."""
    chunks = []
    offset = 0
    for p in paragraphs:
        n = p["total_words"]
        chunks.append({
            ...
            "label": p.get("label", f"Chunk {p['paragraph_id']}"),
            "words": words[offset:offset + n],
        })
        offset += n
    return chunks


def compute_chunk_vocab_scores(chunks: list[dict]) -> list[dict]:
    """Compute vocab_level for each paragraph chunk."""
    results = []
    for chunk in chunks:
        representative = compute_representative_levels(chunk["words"])
...
def compute_tier1(student: str, lesson: str) -> dict:
    ...
    # Per-chunk vocab scores (if paragraph metadata exists)
    paragraphs = extract_paragraphs(outputs)
    chunk_scores = None
    if paragraphs:
        chunks = split_words_by_paragraph(all_words, paragraphs)
        chunk_scores = compute_chunk_vocab_scores(chunks)

    # Lesson-level vocab score: mean of chunk scores if available,
    # otherwise global computation as fallback.
    representative = compute_representative_levels(all_words)
    if chunk_scores:
        scores = [c["vocab_level"]["score"] for c in chunk_scores]
        lesson_score = round(sum(scores) / len(scores), 1)
        total_content = sum(c["vocab_level"]["content_words_scored"] for c in chunk_scores)
        if lesson_score < 1.5:
            label = "A1"
        elif lesson_score < 2.5:
            label = "A2"
        elif lesson_score < 3.5:
            label = "B1"
        elif lesson_score < 4.5:
            label = "B2"
        elif lesson_score < 5.5:
            label = "C1"
        else:
            label = "C2"
        vocab_level = {
            "score": lesson_score,
            "cefr_label": label,
            "content_words_scored": total_content,
        }
    else:
        vocab_level = compute_vocab_level(representative)
